...
function App() {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case ViewState.COURSES: return <div className="h-full overflow-y-auto pb-32"><CourseList /></div>;
  ...
  return (
    <div className="h-full flex flex-col bg-[#f8fafb]">
      {currentView !== ViewState.ACCOUNT && (
        <header 
          className="px-6 pb-4 flex justify-between items-center bg-white/80 backdrop-blur-xl sticky top-0 z-40 border-b border-gray-50"
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <button className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 relative active:scale-90 transition-transform">
            ...
            <button onClick={() => setCurrentView(ViewState.ACCOUNT)} className="w-10 h-10 rounded-2xl overflow-hidden border-2 border-white shadow-md active:scale-90 transition-transform">
